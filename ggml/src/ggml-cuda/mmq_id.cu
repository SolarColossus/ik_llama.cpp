...
#pragma unroll
            for (int offset = neu_padded; offset < warp_size; offset += neu_padded) {
                const int tmp = __shfl_up_sync(WARP_MASK, it_compact_add_self, offset, warp_size);
            ...
            it_compact += __shfl_sync(WARP_MASK, it_compact_add_lower + it_compact_add_self, warp_size - 1, warp_size);
        }
    }
